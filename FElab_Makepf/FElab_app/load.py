...
###############################################################################
        ...
        for i in range(opt_rebal_roof+1):
            ...
            opt_date = stock_return["Date"].between(start_datetime,start_datetime + relativedelta(months=rebal_month)+relativedelta(days = -1))]
            # opt_date의 가장처음과 끝의 날짜를 가져온다
            opt_start = opt_date(처음) ####
            opt_end = opt_date(끝) ####
            # 앞서 산출한 종목으로 
            c_m = c_Models(a,b,opt_start,opt_end,conn)
        ...
        for i in range(rebal_roof+1):
            ...
            for j in range(stock_num):
                filtered_df.iloc[:,j+1] = filtered_df.iloc[:,j+1]*float(b[j][i])
                print(b[j][i])
            # 이후 각각의 종목의 비중을 계산해서 산출한다
